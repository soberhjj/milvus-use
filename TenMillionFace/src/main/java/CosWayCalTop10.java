...
/**
 * @Author: huangJunJie  2021-04-22 14:17
 */
public class CosWayCalTop10 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //load new face
        BufferedReader bufferedReader = new BufferedReader(new FileReader("D:\\人脸图片分两组\\group1中每张人脸对应的特征向量.txt"));
        String line1;
        ...
        for (int i = 1; i < 7; i++) {
            BufferedReader br1 = new BufferedReader(new FileReader(String.format("/home/root/accuracyTestFor100wData/testdataset/%d", i)));

            LinkedList<float[]> testSetFeature = new LinkedList<>();
            String lineInTestdata;
            while ((lineInTestdata = br1.readLine()) != null) {
                List<Float> normalize = normalize(parseLine1(lineInTestdata));
            ...
            LinkedList<float[]> topKByCos = calcTopKByCos(list, testSetFeature, 100);
     ...
     * 解析每行数据
     */
    public static List<Float> parseLine1(String line) {
        String[] origin = line.split(" ");
        List<Float> list = Arrays.stream(Arrays.copyOfRange(origin, 1, origin.length - 1)).map((itme) -> Float.parseFloat(itme)).collect(Collectors.toList());
        return list;
    }
