 ...
 */
public class InsertToWriteNode {
    public static void main(String[] args) {
        ConnectParam connectParam = new ConnectParam.Builder().withHost("192.168.136.198").withPort(19539).build();
        MilvusClient client = new MilvusGrpcClient(connectParam);

        List<Long> entityIds = new ArrayList<>();
        entityIds.add(1000003L);

        InsertParam insertParam = new InsertParam.Builder("TestDelay").withFloatVectors(generateVectors(1,512)).withVectorIds(entityIds).build();
